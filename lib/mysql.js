...
let articles =
    `create table if not exists articles(
        ...
        label VARCHAR(200) NOT NULL COMMENT '文章标签',
        primary key(postid)
    );`
...
let findUserIdData = (ids) => {
    let _sql = `select * from users where id="${ids}";`
    return query(_sql)
}
...
let insertPost = function (value) {
    let _sql = `insert into articles set uid=?,avator=?,name=?,title=?,moment=?,pv=?,md=?,label=?`
    return query(_sql, value)
}
// 更新文章
let updatePost = function (value) {
    let _sql = `update articles set title=?,md=?,label=?,moment=? where postid=?`
    return query(_sql, value)
}
// 删除文章
let deletePost = function (id) {
    let _sql = `delete from articles where postid=${id}`;
    return query(_sql)
}
// 查询分页文章
let findAllPost = function (value) {
    let size = Number(value.pageSize);
    let _sql = ` select * FROM articles order by postid desc limit ${(value.page-1)*value.pageSize}, ${value.pageSize};`
    console.log(_sql)
    return query(_sql)
}

// 查询文章详情
let findPostDetail = function (value) {
    let _sql = ` select * FROM articles where postid = ${value};`
    return query(_sql)
}
// 查询所有文章
let findPostTotal = function () {
    let _sql = `select count(*) as total FROM articles;`
    return query(_sql)
}
// 通过文章的名字查找用户
